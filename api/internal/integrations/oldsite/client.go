...
//
// Контракт вскрыт из /js/script.js старого сайта (read-only):
...
func (c *Client) Deliver(ctx context.Context, kind string, payload []byte) error {
	...
	case orders.KindOrder:
		// На старом сайте оформление заказа шло отдельным контуром корзины;
		// его приёмник ещё не вскрыт, а checkout нового сайта пока без макета.
		// Заказы не теряются: остаются в outbox с ошибкой и видны в админке.
		return fmt.Errorf("oldsite: доставка заказов через мост пока не реализована (нет контракта checkout)")
	default:
