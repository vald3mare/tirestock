...
for _ in $(seq 1 60); do
  if docker compose -f infra/docker-compose.prod.yml exec -T api wget -qO- http://localhost:8080/api/v1/health >/dev/null 2>&1; then
...
$COMPOSE ps
echo
echo "Готово. Витрина: https://$(grep -E '^DOMAIN=' infra/.env.prod | cut -d= -f2)"
